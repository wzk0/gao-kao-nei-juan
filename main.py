...
if platform.system() == 'Windows':
	print("系统为:" + platform.system() + "，不适合学习！请换个Linux系统进行学习!")
else:
...
if 模式=='1':
	...
	if mode=='0':
		print("答题模式:顺序")
		print("\033[36m按照顺序从所有科目中选择题目.\033[0m")
		number=0
		while number<=b:
			act="wget "+rf.repo+str(number)+".yaml"
			os.system(act)
			...
			number+=1
			if b-number+1!=0:
				print('还剩余\033[36m'+str(b-number+1)+'\033[0m道题,继续加油啊!\n')
				conti=input('是/否(y/n)继续:')
				if conti=='n':
					print('Bye~')
					sys.exit(0)
				else:
...
if 模式=="a":
	...
	time.sleep(1.5)
	print('尽管现在题库数量很少,而且只有我一个贡献者...但是!')
	time.sleep(1.5)
	print('质量还是可以保证的!!(毕竟大部分是我自己的错题...🌚)')
	time.sleep(1.5)
	print("\n* 关于积分")
	time.sleep(1.5)
	print("(积分其实是没有用的..某种意义上，因为你不能用积分换钱💰，或者从我手里换些什么东西.")
	time.sleep(1.5)
	print("不过，你可以将你的积分数目截图发给我(联系方式在博客: https://wzk0.github.io/about)")
	time.sleep(1.5)
	print("在我闲(或者投送人数比较多)的时候，我会做一个积分榜单，供大家'攀比'！")
	time.sleep(1.5)
